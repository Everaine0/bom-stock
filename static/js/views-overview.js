...
window.Views.overview = {
  async render(container) {
    ...
    let out = '<div class="grid-stats">' +
      ...
      '<div class="stat"><div class="k">低库存（≤' + lsTh + ' 件）</div><div class="v ' + ((st.low_stock_count || 0) > 0 ? 'red' : '') + '">' + U.fmtNum(st.low_stock_count) + '</div></div>' +
    ...
    out += '<div class="card" style="max-width:620px"><h2>元件类别分布</h2><div class="table-wrap"><table><thead><tr><th>类别</th><th class="num">种数</th><th class="num">数量</th></tr></thead><tbody>' + catRows + '</tbody></table></div></div>';
  ...
  trendHtml(trend) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      bars += '<rect x="' + (cx - bw - 1.5) + '" y="' + (padT + viewH - hC) + '" width="' + bw + '" height="' + hC + '" fill="var(--warn)" opacity=".85" rx="2"/>' +
        '<rect x="' + (cx + 1.5) + '" y="' + (padT + viewH - hR) + '" width="' + bw + '" height="' + hR + '" fill="var(--green)" opacity=".85" rx="2"/>';
      pts += (i ? ',' : '') + cx.toFixed(1) + ',' + yAt(t.profit).toFixed(1);
      dots += '<circle cx="' + cx.toFixed(1) + '" cy="' + yAt(t.profit).toFixed(1) + '" r="2.5" fill="var(--accent)"/>';
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;max-width:' + W + 'px" xmlns="http://www.w3.org/2000/svg">' +
      grid + bars + (pts ? '<polyline points="' + pts + '" fill="none" stroke="var(--accent)" stroke-width="2"/>' + dots : '') + labels + '</svg>' +
      '<div class="small muted" style="margin-top:6px"><span style="color:var(--warn)">▍成本</span> <span style="color:var(--green)">▍收益</span> <span style="color:var(--accent)">— 毛利</span></div>';
  },
